...
// 1. 平均情況 (Average Case) = O(n log n)
// 在大部分情況下，Quick Sort 的平均時間複雜度為 O(n log n)。這是因為每次遞迴會將數組分割為大約相等的兩部分，並進行 `log n` 次分割。對每一層分割來說，都需要掃描所有的 `n` 個元素，因此平均時間複雜度為 `O(n log n)`。

// 2. 最壞情況 (Worst Case)  = O(n^2)
// 最壞情況發生在每次選擇的 `pivot` 都是數組中的最大或最小值，這會導致 Quick Sort 的分割效果很差，使遞迴深度達到 `n`。因此，在最壞情況下，Quick Sort 的時間複雜度為 O(n^2)。
// 例如，當數組本身已經排序或反向排序時，如果每次選擇的 `pivot` 恰好是最左或最右邊的元素，則 Quick Sort 每次分割都只減少一個元素，最壞時間複雜度會是 `O(n^2)`。

// 3. 最好情況 (Best Case) = O(n log n)
// 在理想情況下，每次選擇的 `pivot` 都能將數組均勻分成兩半。當數組
